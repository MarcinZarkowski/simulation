...
def _run_day(
    ...
    what step 4 exposes; step 5 submits; steps 7-9 happen in end_bar.
    """
    ...
    end_context_positions = None
    for i, (engine, strategy) in enumerate(zip(engines, strategies, strict=True)):
        end_context_positions = engine.positions()
        strategy.on_session_end(Context(
            timestamp=session_day,
            account=engine.account_state(),
            positions=end_context_positions,
            equity_positions=engine.equity_positions(),
            contracts=contracts,
            session_day=session_day,
            scenario_id=i,
